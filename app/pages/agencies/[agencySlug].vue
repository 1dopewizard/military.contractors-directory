  ...
  return [
    {
      ...
      detail: data.value.fiscalYears.length
        ? `FY${data.value.fiscalYears.join(", FY")}`
        : null,
    },
    {
      label: "Award records",
...
<template>
  <main class="min-h-full">
    <DirectoryBreadcrumb
      :extra="[{ label: data.agency.name }]"
      :freshness="data.sourceMetadata?.cacheStatus || null"
    />

    <DirectoryPageHeader eyebrow="Agency" :title="data.agency.name">
      <template #actions>
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <template v-else>
        ...
        >
          ...
          <p class="text-muted-foreground mt-1 text-sm">
            Source records supporting the ranking above.
          </p>
