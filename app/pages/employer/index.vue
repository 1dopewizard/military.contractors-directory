...
const { data: profile, pending: profileLoading, error: profileError, refresh } = await useFetch('/api/employer/profile')
...
<template>
  ...
  <div v-else-if="!profile" class="min-h-screen flex items-center justify-center">
    <Card class="p-6 text-center max-w-md">
      ...
      <Button as-child>
        <NuxtLink to="/employer/claim">Claim Your Profile</NuxtLink>
      </Button>
    </Card>
  </div>
