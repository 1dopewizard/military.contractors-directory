...
onMounted(async () => {
  ...
  try {
    ...
    if (user.value) {
      logger.info({ userId: user.value.id, email: userEmail.value }, 'AuthCallback: User authenticated')
      
      // If there's a redirect parameter, use that
      if (redirectTo.value) {
        logger.info({ redirectPath: redirectTo.value }, 'AuthCallback: Redirecting to intended destination')
        setTimeout(() => {
      ...
      if (isAdmin) {
        redirectPath = '/admin'
        logger.info({ email: userEmail.value, redirectPath, isAdmin }, 'AuthCallback: Admin user authenticated, redirecting')
      } else {
        logger.info({ email: userEmail.value, redirectPath }, 'AuthCallback: User authenticated, redirecting to advertiser dashboard')
      }
  ...
  <div class="flex items-center justify-center min-h-[60vh]">
    <Card class="w-full max-w-md">
      <CardContent class="pt-6 text-center space-y-4">
        <div v-if="loading">
          <Spinner class="size-12 text-primary mx-auto" />
          <div class="space-y-2">
        ...
        <div v-else-if="error">
          <Icon name="mdi:alert-circle" class="w-12 h-12 mx-auto text-red-600 dark:text-red-400" />
          <div class="space-y-2">
            <h2 class="text-xl font-semibold text-red-600 dark:text-red-400">Authentication Failed</h2>
            <p class="text-gray-600 dark:text-gray-400">
              {{ error }}
            </p>
            <div class="pt-4">
