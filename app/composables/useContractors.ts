...
export const useContractors = (): UseContractorsReturn => {
  ...
  const getAllContractors = async (): Promise<Contractor[]> => {
    ...
    if (allContractors.value.length > 0) {
      logger.debug({ count: allContractors.value.length }, 'Returning cached contractors')
    ...
    try {
      ...
      logger.debug({ count: contractors.length }, 'Contractors fetched and cached')
      return contractors
    } catch (error: unknown) {
  ...
  const getContractorById = async (id: string): Promise<Contractor | undefined> => {
  ...
  const getContractorBySlug = async (slug: string): Promise<Contractor | undefined> => {
    logger.debug({ slug }, 'getContractorBySlug called')
    
    try {
      const data = await $fetch<Contractor>(`/api/contractors/${slug.toLowerCase()}`)
      ...
      const message = error instanceof Error ? error.message : String(error)
      logger.error({ error: message, slug }, 'Failed to fetch contractor by slug')
      return undefined
    }
  }

  const searchContractors = async (query: string, limit = 25): Promise<Contractor[]> => {
    ...
    try {
      ...
      logger.debug({ query: trimmed, resultCount: data?.length || 0 }, 'searchContractors completed')
      return data || []
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error)
  ...
  const filterContractors = async (filters: ContractorFilters = {}): Promise<Contractor[]> => {
    ...
    try {
      ...
      if (specialty && specialty.trim().length > 0) {
        filtered = filtered.filter(c => 
          c.specialties?.some(s => s.slug === specialty || s.name === specialty)
        )
      }
      
      // Sort
      if (sort === 'rank') {
        filtered.sort((a, b) => (a.defenseNewsRank || 999) - (b.defenseNewsRank || 999))
      } else if (sort === 'revenue') {
        filtered.sort((a, b) => (b.defenseRevenue || 0) - (a.defenseRevenue || 0))
      } else {
        // Default: sort alphabetically by name
        filtered.sort((a, b) => a.name.localeCompare(b.name))
      }

      logger.debug({ filters, resultCount: filtered.length }, 'filterContractors completed')
      return filtered
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : String(error)
