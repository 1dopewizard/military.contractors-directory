...
export const useBases = () => {
  ...
  const fetchBases = async (options?: {
    ...
    try {
      const response = await $fetch<{ bases: BaseWithTheater[] }>(
        "/api/bases",
        {
          query: {
      ...
      logger.info(
        { count: transformedBases.length, ...options },
        "Bases fetched",
      );
      return transformedBases;
    } catch (err) {
      const message =
        err instanceof Error ? err.message : "Failed to fetch bases";
      error.value = message;
  ...
  const fetchBase = async (
    ...
    try {
      const data = await $fetch<BaseDetailResponse>(
        `/api/bases/${slug.toLowerCase()}`,
        {
          query: {
            limit: options?.limit,
      ...
      const message =
        err instanceof Error ? err.message : "Failed to fetch base";
      error.value = message;
   ...
   */
  const getBasesByCountry = (
    theaterCode?: string,
  ): Map<string, BaseWithTheater[]> => {
