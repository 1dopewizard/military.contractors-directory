...
export const useAuth = () => {
  ...
  const sessionData = computed(
    () => (sessionRef as any).value?.data ?? (sessionRef as any).data,
  );
  const sessionPending = computed(
    () =>
      (sessionRef as any).value?.isPending ??
      (sessionRef as any).isPending ??
      false,
  );
  ...
  const refreshSession = async () => {
    const refreshFn =
      (sessionRef as any).value?.refresh ?? (sessionRef as any).refresh;
