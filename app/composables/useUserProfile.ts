...
export const useUserProfile = () => {
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      if (response.user) {
        ...
        logger.info({ userId: response.user.id }, 'Profile loaded successfully')
        return response
      } else {
  ...
  const updateProfile = async (updates: ProfileUpdate) => {
    ...
    try {
      ...
      logger.error({ error: errorMessage }, 'Unexpected error updating profile')
  ...
  const uploadAvatar = async (_file: File) => {
    ...
    return { success: false, error: 'Avatar upload not yet implemented', url: null }
  }
