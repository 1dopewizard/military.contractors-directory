...
<template>
  <div class="border-border border">
    <div v-if="!awards.length" class="text-muted-foreground p-8 text-center text-sm">
      {{ emptyText }}
    </div>
    <div v-else class="divide-border divide-y">
      <article v-for="award in awards" :key="award.key" class="p-4">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div class="min-w-0">
            <p class="text-foreground text-sm font-medium [overflow-wrap:anywhere]">
              {{ award.recipientName }}
            </p>
            <p class="text-muted-foreground mt-1 text-sm leading-relaxed [overflow-wrap:anywhere]">
              {{ award.description || "No description provided." }}
            </p>
          </div>
          <p class="text-foreground shrink-0 text-sm font-semibold tabular-nums">
        ...
        <div class="text-muted-foreground mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs">
          ...
          <span>{{ award.awardingSubAgency || award.awardingAgency || "Agency N/A" }}</span>
