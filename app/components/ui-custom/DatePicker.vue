...
watch(() => props.modelValue, (newVal) => {
  ...
  try {
    ...
    const datePart = normalizedValue.includes('T') ? normalizedValue.split('T')[0] : normalizedValue
  ...
  <Popover>
    <PopoverTrigger as-child>
      <Button
        variant="ghost"
        :class="cn(
          'w-full justify-start text-left font-normal bg-muted/30 border-border/50 h-10',
          !value && 'text-muted-foreground'
        )"
      >
        <CalendarIcon class="mr-2 h-4 w-4" />
        {{ value ? df.format(value.toDate(getLocalTimeZone())) : (placeholder || "Pick a date") }}
      </Button>
    </PopoverTrigger>
