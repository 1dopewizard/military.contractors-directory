...
const statusOptions = [
  { value: 'prospect', label: 'Prospect', color: 'bg-slate-500/10 text-slate-600 dark:text-slate-400' },
  { value: 'contacted', label: 'Contacted', color: 'bg-blue-500/10 text-blue-600 dark:text-blue-400' },
  { value: 'engaged', label: 'Engaged', color: 'bg-amber-500/10 text-amber-600 dark:text-amber-400' },
  { value: 'active', label: 'Active', color: 'bg-green-500/10 text-green-600 dark:text-green-400' },
  { value: 'inactive', label: 'Inactive', color: 'bg-gray-500/10 text-gray-600 dark:text-gray-400' }
]
...
const fetchContacts = async () => {
  ...
  try {
    const response = await $fetch<{ contacts: HRContact[] }>('/api/admin/employer-contacts', {
      query: {
  ...
  try {
    await $fetch(`/api/admin/employer-contacts/${contact.id}`, {
      ...
      body: {
        ...
        relationship_status: contact.relationship_status === 'prospect' ? 'contacted' : contact.relationship_status
      }
    })
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between flex-wrap gap-4">
      <div>
        <h2 class="text-sm font-semibold text-foreground uppercase tracking-wide">HR Contacts</h2>
        <p class="text-sm text-muted-foreground">Manage employer relationships for placements</p>
      </div>
    ...
    <Card v-else-if="error" class="p-6 text-center border-destructive/30">
      ...
      <Button variant="ghost" size="sm" class="mt-4" @click="fetchContacts">Try Again</Button>
    </Card>

    <!-- Empty -->
    <Empty v-else-if="contacts.length === 0" class="border">
      <EmptyMedia variant="icon">
        <Icon name="mdi:account-tie-outline" class="w-6 h-6" />
      </EmptyMedia>
      <EmptyTitle class="text-base">No contacts found</EmptyTitle>
      <EmptyDescription>Add your first HR contact to get started.</EmptyDescription>
    ...
    <div v-else class="divide-y divide-border/30">
      <div v-for="contact in contacts" :key="contact.id" class="py-3 first:pt-0">
        <div class="flex items-center gap-4">
          <!-- Contact Info -->
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="font-medium text-sm">{{ contact.contact_name }}</span>
              <span :class="getStatusColor(contact.relationship_status)" class="px-1.5 py-0.5 text-[10px] font-medium rounded">
                {{ statusOptions.find(s => s.value === contact.relationship_status)?.label }}
              </span>
              <Badge v-if="contact.title" variant="secondary" class="text-[10px]">
                {{ contact.title }}
              </Badge>
            </div>
            <p class="text-xs text-muted-foreground">{{ contact.company_name }}</p>
            <div v-if="contact.email || contact.phone" class="flex flex-wrap gap-2 mt-1">
              <span v-if="contact.email" class="text-xs text-muted-foreground">
                {{ contact.email }}
              </span>
              <span v-if="contact.phone" class="text-xs text-muted-foreground">
                {{ contact.phone }}
              </span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-1">
            <Button variant="ghost" size="sm" class="h-7 w-7 p-0" title="Log contact" @click="logContact(contact)">
              <Icon name="mdi:phone-check" class="w-4 h-4" />
            </Button>
            <Button variant="ghost" size="sm" class="h-7 w-7 p-0" title="Edit" @click="openDialog(contact)">
              <Icon name="mdi:pencil-outline" class="w-4 h-4" />
            </Button>
            <Button variant="ghost" size="sm" class="h-7 w-7 p-0 text-destructive" title="Delete" @click="deleteContact(contact.id)">
    ...
    <Dialog v-model:open="isDialogOpen">
      <DialogContent class="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {{ editingContact ? 'Update contact information' : 'Add a new HR contact for employer outreach' }}
          </DialogDescription>
        </DialogHeader>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4 py-2">
          <div class="grid sm:grid-cols-2 gap-4">
            <div class="space-y-1.5">
              ...
              <Input
                ...
                @update:model-value="(v) => form.setFieldValue('company_name', String(v))"
                placeholder="Lockheed Martin"
              />
              <p v-if="formSubmitted && form.errors.value.company_name" class="text-xs text-destructive">
            ...
            <div class="space-y-1.5">
              ...
              <Input
                ...
                @update:model-value="(v) => form.setFieldValue('contact_name', String(v))"
                placeholder="Jane Smith"
              />
              <p v-if="formSubmitted && form.errors.value.contact_name" class="text-xs text-destructive">
          ...
          <div class="grid sm:grid-cols-2 gap-4">
            <div class="space-y-1.5">
              ...
              <Input
                ...
                @update:model-value="(v) => form.setFieldValue('title', String(v))"
            ...
            <div class="space-y-1.5">
              ...
              <Select
                ...
                @update:model-value="(v) => form.setFieldValue('relationship_status', String(v))"
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <div class="space-y-1.5">
            ...
            <Input
              ...
              @update:model-value="(v) => form.setFieldValue('linkedin_url', String(v))"
          ...
          <div class="flex gap-2 pt-2">
            <Button type="button" variant="ghost" class="flex-1" @click="isDialogOpen = false">
              Cancel
            </Button>
