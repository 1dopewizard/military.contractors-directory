...
const fetchRecruiters = async () => {
  ...
  try {
    const response = await $fetch<{ recruiters: Recruiter[] }>(
...
const toggleActive = async (recruiter: Recruiter) => {
  try {
    ...
    toast.success(
      recruiter.is_active ? "Recruiter deactivated" : "Recruiter activated",
    );
    await fetchRecruiters();
  } catch (err: any) {
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p class="text-muted-foreground text-sm">
          Manage team members with recruiter access
        </p>
      </div>
      <div class="flex items-center gap-2">
        ...
        <Button size="sm" @click="isInviteDialogOpen = true">
          <Icon name="mdi:account-plus" class="mr-1 h-4 w-4" />
          Add Recruiter
        </Button>
      </div>
    </div>
    ...
    <Card v-else-if="error" class="border-destructive/30 p-6 text-center">
      ...
      <Button variant="ghost" size="sm" class="mt-4" @click="fetchRecruiters"
        >Try Again</Button
      >
    </Card>
    ...
    <Empty v-else-if="recruiters.length === 0" class="border">
      <EmptyMedia variant="icon">
        <Icon name="mdi:account-group-outline" class="h-6 w-6" />
      </EmptyMedia>
      <EmptyTitle class="text-base">No recruiters yet</EmptyTitle>
      <EmptyDescription
        >Add your first recruiter to get started.</EmptyDescription
      >
      <Button size="sm" class="mt-4" @click="isInviteDialogOpen = true">
        <Icon name="mdi:account-plus" class="mr-1.5 h-4 w-4" />
        Add Recruiter
      </Button>
    </Empty>
      ...
      >
        <div class="flex items-center gap-4">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p v-if="recruiter.name" class="text-muted-foreground text-xs">
              {{ recruiter.email }}
            </p>
          ...
          <div class="flex items-center gap-1">
            ...
            >
              <Icon
                :name="
                  recruiter.is_active ? 'mdi:account-off' : 'mdi:account-check'
                "
                class="h-4 w-4"
              />
            </Button>
    ...
    <Dialog v-model:open="isInviteDialogOpen">
      <DialogContent class="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Add a team member with recruiter access. They will be able to view
            candidates, manage placements, and contact employers.
          </DialogDescription>
        </DialogHeader>
        ...
        <form @submit.prevent="handleInvite" class="space-y-4 py-2">
          ...
          <div class="flex gap-2 pt-2">
            ...
            >
              <Spinner v-if="isInviting" class="mr-2 h-4 w-4" />
              Add Recruiter
            </Button>
          </div>
        </form>
