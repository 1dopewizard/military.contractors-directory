...
<template>
  <div
    class="space-y-3 border-l-2 border-amber-500/30 bg-amber-500/5 px-4 py-3"
  >
    <!-- Header -->
    ...
    <div class="text-muted-foreground space-y-1 text-sm">
      <template v-if="isSpotlight">
        <p>
          <span class="text-foreground font-medium">Tagline:</span>
          {{ spotlight.tagline }}
        </p>
        <p>
        ...
        <p>
          <span class="text-foreground font-medium">CTA:</span>
          {{ spotlight.cta_text }}
        </p>
        <p>
      ...
      <template v-else>
        ...
        <p>
          <span class="text-foreground font-medium">Clearance:</span>
          {{ job.clearance }}
        </p>
        <p>
          <span class="text-foreground font-medium">Salary:</span>
          {{ job.salary }}
        </p>
        <p>
          <span class="text-foreground font-medium">Pitch:</span>
          {{ job.pitch }}
        </p>
        <p>
          <span class="text-foreground font-medium">Apply URL:</span>
          <a
            :href="job.apply_url"
            target="_blank"
            class="text-primary hover:underline"
    ...
    <div class="flex items-center gap-2">
      <Button size="sm" @click="handleApprove">
        <Icon name="mdi:check" class="mr-1 h-4 w-4" />
        Approve
      </Button>
      <Dialog v-model:open="showRejectDialog">
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Provide a reason for rejecting this ad. This will be visible to
              the advertiser.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="ghost" @click="showRejectDialog = false"
              >Cancel</Button
            >
            <Button
              variant="destructive"
