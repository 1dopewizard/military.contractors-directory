...
const spotlightSchema = toTypedSchema(
  z.object({
    ...
    tagline: z
      ...
      .max(50, "Tagline must be under 50 characters"),
    headline: z
      .string()
      .min(10, "Headline is required")
...
const previewData = computed(() => ({
  ...
  description:
    form.values.description || "Your description will appear here...",
      ...
      <div class="grid gap-4">
        <div class="grid gap-4 sm:grid-cols-2">
          <div class="space-y-1.5">
            ...
            >
            <Input
              :model-value="form.values.advertiser"
              @update:model-value="
                (v) => form.setFieldValue('advertiser', String(v))
              "
              placeholder="e.g. CACI International"
              class="h-10"
              maxlength="100"
            />
            <div class="flex justify-between">
              <p
                v-if="formSubmitted && form.errors.value.advertiser"
                class="text-destructive text-xs"
              >
                {{ form.errors.value.advertiser }}
              </p>
              <p class="text-muted-foreground ml-auto text-[10px]">
                {{ form.values.advertiser?.length || 0 }}/100
              </p>
            </div>
          </div>
          <div class="space-y-1.5">
            ...
            >
            <Input
              :model-value="form.values.tagline"
              @update:model-value="
                (v) => form.setFieldValue('tagline', String(v))
              "
              placeholder="e.g. Careers for the Mission-Driven"
              class="h-10"
              maxlength="50"
            />
            <div class="flex justify-between">
              <p
                v-if="formSubmitted && form.errors.value.tagline"
                class="text-destructive text-xs"
              >
                {{ form.errors.value.tagline }}
              </p>
              <p class="text-muted-foreground ml-auto text-[10px]">
                {{ form.values.tagline?.length || 0 }}/50
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-1.5">
          ...
          >
          <Input
            :model-value="form.values.headline"
            @update:model-value="
              (v) => form.setFieldValue('headline', String(v))
            "
            placeholder="e.g. Join 23,000+ professionals protecting national security"
            class="h-10"
            maxlength="80"
          />
          <div class="flex justify-between">
            <p
              v-if="formSubmitted && form.errors.value.headline"
              class="text-destructive text-xs"
            >
              {{ form.errors.value.headline }}
            </p>
            <p class="text-muted-foreground ml-auto text-[10px]">
              {{ form.values.headline?.length || 0 }}/80
            </p>
          </div>
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <Textarea
            ...
            @update:model-value="
              (v) => form.setFieldValue('description', String(v))
            "
          ...
          <div class="flex justify-between">
            <p
              v-if="formSubmitted && form.errors.value.description"
              class="text-destructive text-xs"
            >
              {{ form.errors.value.description }}
            </p>
            <p class="text-muted-foreground ml-auto text-[10px]">
              {{ form.values.description?.length || 0 }}/200
            </p>
          </div>
        </div>
        ...
        <div class="space-y-1.5">
          <Label class="text-muted-foreground text-xs tracking-wider uppercase"
            >Industries *
            <span class="font-normal normal-case">(select up to 3)</span></Label
          >
          <div class="flex flex-wrap gap-2">
            <Button
              ...
              size="sm"
              :variant="
                form.values.industries?.includes(industry)
                  ? 'default'
                  : 'outline'
              "
              class="h-8 text-xs"
              :disabled="
                !form.values.industries?.includes(industry) &&
                (form.values.industries?.length || 0) >= 3
              "
          ...
          <p
            v-if="formSubmitted && form.errors.value.industries"
            class="text-destructive text-xs"
          >
            {{ form.errors.value.industries }}
          </p>
        </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <div class="space-y-1.5">
            ...
            >
            <Input
              :model-value="form.values.cta_text"
              @update:model-value="
                (v) => form.setFieldValue('cta_text', String(v))
              "
            ...
            <div class="flex justify-between">
              <p
                v-if="formSubmitted && form.errors.value.cta_text"
                class="text-destructive text-xs"
              >
                {{ form.errors.value.cta_text }}
              </p>
              <p class="text-muted-foreground ml-auto text-[10px]">
                {{ form.values.cta_text?.length || 0 }}/25
              </p>
            </div>
          </div>
          <div class="space-y-1.5">
            ...
            <div class="relative">
              ...
              />
              <Input
                :model-value="form.values.cta_url"
                @update:model-value="
                  (v) => form.setFieldValue('cta_url', String(v))
                "
            ...
            <p
              v-if="formSubmitted && form.errors.value.cta_url"
              class="text-destructive text-xs"
            >
              {{ form.errors.value.cta_url }}
            </p>
          </div>
        </div>
      </div>
