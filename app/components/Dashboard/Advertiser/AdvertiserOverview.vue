...
<template>
  <div class="space-y-8">
    ...
    <div class="grid grid-cols-2 gap-6 md:grid-cols-3 lg:grid-cols-6">
      ...
      <button
        class="hover:bg-muted/30 -mx-2 space-y-1 px-2 text-left transition-colors"
      ...
      <div class="space-y-1">
        ...
        <p class="text-muted-foreground text-xs tracking-wide uppercase">CTR</p>
      </div>
    </div>
    ...
    <div class="space-y-3">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <Button variant="ghost" size="sm" @click="emit('setTab', 'my-ads')">
          <Icon name="mdi:view-list-outline" class="mr-2 h-4 w-4" />
          View All Ads
        </Button>
    ...
    <!-- Performance Summary -->
    <div
      v-if="stats.totalImpressions > 0"
      class="bg-primary/5 border-primary/10 border p-4"
    >
      <div class="flex items-start gap-3">
        ...
        />
        <div>
          <p class="text-foreground text-sm font-medium">Performance Summary</p>
          <p class="text-muted-foreground mt-1 text-xs">
            Your ads have received
            {{ stats.totalImpressions.toLocaleString() }} impressions and
            {{ stats.totalClicks.toLocaleString() }} clicks with a {{ ctr }}%
            click-through rate.
          </p>
        </div>
      </div>
    ...
    <div class="space-y-3">
      ...
      <div v-if="isLoading" class="space-y-2">
        <div v-for="i in 3" :key="i" class="bg-muted/30 h-16 animate-pulse" />
      </div>
        ...
        <Empty>
          <EmptyMedia>
            <Icon
              ...
              class="text-muted-foreground/50 size-12"
            />
          </EmptyMedia>
          <EmptyTitle>No ads yet</EmptyTitle>
          <EmptyDescription>
            Create your first featured ad to reach thousands of cleared
            veterans.
          </EmptyDescription>
        </Empty>
        ...
        >
          <div class="min-w-0 flex-1">
            <div class="mb-0.5 flex items-center gap-2">
              <span
                class="px-1.5 py-0.5 text-[10px] font-medium tracking-wider uppercase"
                :class="
                  item.type === 'job'
          ...
          <div class="shrink-0 text-right">
            ...
            <p class="text-muted-foreground text-[10px]">impressions</p>
          </div>
        </div>
      </div>
    ...
    <div
      v-if="stats.total === 0"
      class="border-muted-foreground/25 border border-dashed p-4"
    >
      <div class="flex items-start gap-3">
        <Icon
        ...
        <div>
          ...
          <p class="text-muted-foreground mt-1 text-xs">
            Featured ads get 10x more visibility than standard listings. Start
            with a featured job posting or company spotlight.
          </p>
          <Button
            variant="link"
