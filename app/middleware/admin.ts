...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  if (!isAuthenticated.value) {
    logger.warn({ path: to.path }, 'Unauthorized admin access attempt - not authenticated')
    return navigateTo('/')
  }

  // Check if user is admin
  if (!isAdminEmail(userEmail.value)) {
    logger.warn({ email: userEmail.value, path: to.path }, 'Unauthorized admin access attempt')
    return navigateTo('/')
  }

  logger.info({ email: userEmail.value, path: to.path }, 'Admin access granted')
})
