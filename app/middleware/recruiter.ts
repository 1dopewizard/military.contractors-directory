...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  if (!isAuthenticated.value || !userEmail.value) {
    logger.warn({ path: to.path }, 'Unauthorized recruiter access attempt - not authenticated')
    return navigateTo('/')
  }
  ...
  if (isAdminEmail(email)) {
    logger.info({ email, path: to.path, role: 'admin' }, 'Recruiter access granted (admin)')
    return
  }

  // Check recruiterAccess table via API for non-admin users
  try {
    const result = await $fetch<{ hasAccess: boolean }>('/api/admin/recruiters/check-access', {
      method: 'GET',
      query: { email: email.toLowerCase() },
    })

    if (result.hasAccess) {
      logger.info({ email, path: to.path, role: 'recruiter' }, 'Recruiter access granted')
      return
    }
  } catch {
  ...
  logger.warn({ email, path: to.path }, 'Unauthorized recruiter access attempt')
  return navigateTo('/')
})
