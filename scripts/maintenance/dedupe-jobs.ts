...
async function main() {
  ...
  console.log(`  Unique externalId+sourceSite keys: ${duplicateInfo.uniqueKeys}`)
  console.log(`  Duplicate jobs to remove: ${duplicateInfo.duplicateCount}`)
  console.log(`  Expected count after dedup: ${duplicateInfo.expectedAfterDedup}`)
  console.log()
  ...
  console.log(`Found ${duplicateInfo.duplicateGroups.length} duplicate groups:`)
  ...
  if (dryRun) {
    console.log()
    console.log('This was a dry run. Run without --dry-run to actually delete duplicates.')
  } else {
    console.log()
