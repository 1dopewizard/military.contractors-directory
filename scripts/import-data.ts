...
async function importTheaters() {
  ...
  if (data.length === 0) {
    ...
    const defaultTheaters = [
      ...
      { code: 'INDOPACOM', name: 'US Indo-Pacific Command', region: 'Asia-Pacific' },
      { code: 'AFRICOM', name: 'US Africa Command', region: 'Africa' },
      { code: 'NORTHCOM', name: 'US Northern Command', region: 'North America' },
      { code: 'SOUTHCOM', name: 'US Southern Command', region: 'South America' },
    ]
...
async function importBases() {
  ...
  for (const row of data) {
    ...
    const coords = row.coordinates as { lat?: number; lng?: number } | undefined
...
async function importMosCodes() {
  ...
  if (data.length === 0) {
    ...
    for (const branch of branches) {
      const branchData = readJsonFile<Record<string, unknown>>(`mos_codes_${branch}.json`)
  ...
  for (const row of data) {
    ...
    await db.insert(schema.mosCode).values({
      ...
      clearanceProfile: row.clearance_profile as Record<string, unknown> | undefined,
      deploymentProfile: row.deployment_profile as Record<string, unknown> | undefined,
      seniorityDistribution: row.seniority_distribution as Record<string, unknown> | undefined,
      payBandHint: row.pay_band_hint as string | undefined,
      commonCerts: row.common_certs as string[] | undefined,
      recommendedCertsContract: row.recommended_certs_contract as string[] | undefined,
...
async function importJobs() {
  ...
  for (const row of data) {
    ...
    const companyId = oldCompanyId ? idMaps.companies.get(oldCompanyId) : undefined
    ...
    await db.insert(schema.job).values({
      ...
      compensationData: row.compensation_data as Record<string, unknown> | undefined,
      qualificationsData: row.qualifications_data as Record<string, unknown> | undefined,
      ...
      complianceData: row.compliance_data as Record<string, unknown> | undefined,
      postingData: row.posting_data as Record<string, unknown> | undefined,
      militaryMapping: row.military_mapping as Record<string, unknown> | undefined,
...
async function importCompanyMos() {
  ...
  console.log(`  ✅ Imported ${count} company-MOS mappings (skipped ${skipped})`)
}
...
async function importCampaigns() {
  ...
  for (const row of data) {
    ...
    await db.insert(schema.campaign).values({
      ...
      targetingRules: row.targeting_rules as Record<string, unknown> | undefined,
      createdAt: parseTimestamp(row.created_at as string) || new Date(),
      updatedAt: parseTimestamp(row.updated_at as string) || new Date(),
    })
...
async function importFeaturedEmployers() {
  ...
  for (const row of data) {
    ...
    await db.insert(schema.featuredEmployer).values({
      ...
      endsAt: parseTimestamp(row.ends_at as string) || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000),
...
async function importJobAlertSubscriptions() {
  ...
  const data = readJsonFile<Record<string, unknown>>('job_alert_subscriptions.json')
...
async function importRecruiterAccess() {
  ...
  console.log(`  ✅ Imported ${count} recruiter access records (skipped ${skipped} invalid)`)
}
