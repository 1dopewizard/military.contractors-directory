...
const SponsorCategorySchema = z.enum([
  "WILL_SPONSOR",
  "ELIGIBLE_TO_OBTAIN",
  "ACTIVE_ONLY",
  "NOT_SPECIFIED",
...
async function generateJobBatch(
  ...
  console.log(
    `  Generating ${count} jobs${category ? ` (${category})` : ""}...`,
  );
...
async function insertJob(job: GeneratedJob): Promise<string> {
  ...
  const postedAt = new Date(
    now.getTime() - job.postedDaysAgo * 24 * 60 * 60 * 1000,
  );
...
async function main() {
  ...
  if (dryRun) {
    ...
    for (const job of jobs.slice(0, 5)) {
      ...
      console.log(
        `    ${job.location} (${job.theater}) - ${job.clearanceRequired}`,
      );
      console.log(
        `    $${job.salaryMin.toLocaleString()}-$${job.salaryMax.toLocaleString()} | ${job.seniority}`,
      );
    }
    if (jobs.length > 5) {
