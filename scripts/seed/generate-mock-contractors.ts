...
const TOP_100_FILE = resolve(process.cwd(), 'docs/top_100_defense_contractors.md')
const OUTPUT_FILE = resolve(process.cwd(), 'scripts/seed/mock-contractors.json')
...
const SPECIALTIES = [
  { slug: 'aerospace-defense', name: 'Aerospace & Defense', description: 'Aircraft, missiles, satellites' },
  { slug: 'cybersecurity-it', name: 'Cybersecurity & IT', description: 'Cyber, IT services, cloud' },
  { slug: 'intelligence-analytics', name: 'Intelligence & Analytics', description: 'Intel, data analytics, AI/ML' },
  { slug: 'land-systems', name: 'Land Systems', description: 'Vehicles, weapons, munitions' },
  { slug: 'naval-maritime', name: 'Naval & Maritime', description: 'Ships, submarines, maritime systems' },
  { slug: 'space-systems', name: 'Space Systems', description: 'Satellites, launch, space tech' },
  { slug: 'professional-services', name: 'Professional Services', description: 'Consulting, engineering services' },
  { slug: 'logistics-support', name: 'Logistics & Support', description: 'Base ops, supply chain, MRO' },
  { slug: 'electronics-sensors', name: 'Electronics & Sensors', description: 'Sensors, communications, EW' },
  { slug: 'research-development', name: 'Research & Development', description: 'FFRDCs, labs, R&D' },
]

// U.S. companies from Top 100 (48 total) - normalized for matching
const US_COMPANIES = [
  'Lockheed Martin', 'RTX', 'Northrop Grumman', 'General Dynamics', 'Boeing',
  'L3Harris', 'Leidos', 'HII', 'Amentum', 'Booz Allen Hamilton', 'CACI',
  'GE Aerospace', 'Honeywell', 'KBR', 'Advanced Technology International',
  'Peraton', 'Parsons', 'SpaceX', 'V2X', 'SAIC', // SAIC = Science Applications International Corporation
  'Textron', 'TransDigm', 'Bechtel', 'Oshkosh', 'BWX Technologies',
  'Viasat', 'Sierra Nevada', 'Parker Hannifin', 'Curtiss-Wright',
  'Amphenol', 'Palantir', 'MITRE', 'Moog', 'ManTech',
  'HEICO', 'Howmet Aerospace', 'Aerospace Corporation', 'Keysight',
  'TTM Technologies', 'Teledyne', 'StandardAero', 'Kratos',
  'Spirit AeroSystems', 'Anduril', 'M1 Support Services', 'Mercury Systems',
  'AM General', 'Hexcel'
]
...
function normalizeCompanyName(name: string): string {
  return name
    ...
    .replace(/\s*,\s*(inc|corp|corporation|ltd|llc|technologies|solutions|services)\s*\.?$/i, '') // Remove suffixes
...
const ContractorDataSchema = z.object({
  slug: z.string().describe('URL-safe version of company name (e.g., "lockheed-martin")'),
  description: z.string().min(200).describe('2-3 paragraphs about the company, what they do, their history'),
  headquarters: z.string().describe('Headquarters location (e.g., "Bethesda, Maryland" or "Falls Church, Virginia")'),
  founded: z.number().int().min(1800).max(2025).describe('Year founded'),
  employeeCount: z.string().describe('Approximate employee count (e.g., "45,000" or "~120,000")'),
  website: z.string().url().describe('Official website URL'),
  careersUrl: z.string().url().describe('Careers page URL'),
  linkedinUrl: z.string().url().nullable().describe('LinkedIn company page URL (or null if not available)'),
  wikipediaUrl: z.string().url().nullable().describe('Wikipedia article URL (or null if not available)'),
  stockTicker: z.string().nullable().describe('Stock ticker symbol if public (e.g., "LMT", "RTX") or null if private'),
  isPublic: z.boolean().describe('Whether the company is publicly traded'),
  specialties: z.array(z.string()).min(1).max(3).describe('1-3 specialty slugs from the taxonomy (e.g., ["aerospace-defense", "space-systems"])'),
  keyProducts: z.array(z.string()).min(3).max(5).describe('3-5 notable products/programs (e.g., ["F-35 Lightning II", "THAAD"])'),
  notableContracts: z.array(z.string()).min(2).max(3).describe('2-3 major contract wins or programs (e.g., ["F-35 Production Contract", "Space Force GPS"])'),
})
...
function filterUSCompanies(companies: CompanyFromList[]): CompanyFromList[] {
  return companies.filter(c => {
    ...
    return US_COMPANIES.some(usName => {
      ...
      return normalizedName.includes(normalizedUS) || 
...
async function generateContractorData(
  ...
  const totalRevenueBillions = (company.totalRevenue / 1_000_000_000).toFixed(2)
  const defenseRevenueBillions = (company.defenseRevenue / 1_000_000_000).toFixed(2)
...
async function appendContractor(contractor: GeneratedContractor): Promise<void> {
...
async function main() {
  ...
  if (dryRun) {
    ...
    for (const company of toShow) {
      console.log(`  ${company.name} (#${company.rank}) - $${(company.defenseRevenue / 1_000_000_000).toFixed(2)}B defense revenue`)
    }
    console.log()
  ...
  console.log(`Generating mock data for ${limit ? limit : usCompanies.length} companies...`)
