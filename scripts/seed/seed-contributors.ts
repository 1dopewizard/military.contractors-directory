...
async function linkSubmissionsToContributor(
  ...
  for (const report of unassignedSalaries) {
    await db.update(schema.salaryReport)
      .set({
        submittedBy: userId,
        updatedAt: now,
      })
  ...
  for (const exp of unassignedInterviews) {
    await db.update(schema.interviewExperience)
      .set({
        submittedBy: userId,
        updatedAt: now,
      })
...
async function clearMockContributors(): Promise<ClearResult> {
  ...
  const mockUsers = allUsers.filter(u => (u.email as string).endsWith(MOCK_EMAIL_DOMAIN))
...
function calculateAllocations(
  ...
  // Get shuffled names
  const names = shuffleArray(MOCK_NAMES).slice(0, Math.min(count, MOCK_NAMES.length))
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    let salaries = Math.min(remainingSalaries, Math.max(1, Math.floor(baseSalaries * variance)))
    let interviews = Math.min(remainingInterviews, Math.max(0, Math.floor(baseInterviews * variance)))
    ...
    const emailPrefix = name.toLowerCase().replace(/\s+/g, '_').replace(/\./g, '')
...
function parseArgs(): {
  ...
  return {
    ...
    count: parseInt(args.find(a => a.startsWith('--count='))?.split('=')[1] || '') || DEFAULT_CONTRIBUTOR_COUNT,
  }
}
...
async function main() {
  ...
  if (clear) {
    if (dryRun) {
      ...
      console.log(`  Unlinked ${result.unlinkedInterviews} interview experiences`)
    }
    console.log()
  }
  ...
  if (counts.total === 0) {
    ...
    console.log('   Run seed-community-mock-data.ts first to generate submissions.')
    console.log()
    console.log('='.repeat(60))
    client.close()
    return
  ...
  // Calculate allocations
  const allocations = calculateAllocations(count, counts.unassignedSalaries, counts.unassignedInterviews)

  if (dryRun) {
    console.log('[DRY RUN] Would create contributors:')
    for (const alloc of allocations) {
      console.log(`  ${alloc.name} (${alloc.email}): ${alloc.salaries} salaries, ${alloc.interviews} interviews`)
    }
    console.log()
    const totalLinked = allocations.reduce((sum, a) => sum + a.salaries + a.interviews, 0)
    console.log(`[DRY RUN] Total: ${allocations.length} contributors, ${totalLinked} submissions linked`)
  } else {
    ...
    for (const alloc of allocations) {
      try {
        // Create user
        const userId = await createContributor(alloc.email, alloc.name, alloc.avatarUrl)
        createdCount++
        
        // Link submissions
        const linkResult = await linkSubmissionsToContributor(userId, alloc.salaries, alloc.interviews)
        totalLinked += linkResult.totalLinked
        
        console.log(`  ✓ ${alloc.name}: ${linkResult.linkedSalaries} salaries, ${linkResult.linkedInterviews} interviews`)
      } catch (error) {
