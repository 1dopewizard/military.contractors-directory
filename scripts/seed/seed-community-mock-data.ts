...
const InterviewOutcomeSchema = z.enum([
  "OFFER",
  "REJECTED",
  "GHOSTED",
  "WITHDREW",
...
const SalaryReportSchema = z.object({
  ...
  companySlug: z
    .string()
    .describe(
      "Company slug from: leidos, caci, mantech, booz-allen, saic, peraton, northrop-grumman, lockheed-martin, raytheon, general-dynamics, gdit, l3harris, bae-systems, kbr, amentum, v2x, fluor, parsons, jacobs",
    ),
  location: z
    .string()
    .describe(
      'Location (e.g., "Northern Virginia", "Fort Meade, Maryland", "Stuttgart, Germany")',
    ),
  baseSalary: z
    .number()
    .min(40000)
    .max(300000)
    .describe("Annual base salary in USD"),
...
const InterviewExperienceSchema = z.object({
  ...
  roleTitle: z
    .string()
    .describe(
      'Job title interviewed for (e.g., "Network Engineer III", "Intelligence Analyst")',
    ),
  processDescription: z
    .string()
    .describe("2-4 sentences describing the interview process"),
  questionsAsked: z
    .array(z.string())
    .min(2)
    .max(10)
    .describe("2-4 questions asked during the interview"),
  ...
  timelineWeeks: z
    ...
    .min(1)
    .max(12)
    .describe("Total process length in weeks"),
  helpfulCount: z.number().min(0).max(25).describe("Random helpful vote count"),
});
...
async function generateSalaryReportBatch(
  count: number,
...
async function generateInterviewExperienceBatch(
  count: number,
...
async function generateAllSalaryReports(
  total: number,
): Promise<SalaryReport[]> {
...
async function generateAllInterviewExperiences(
  total: number,
): Promise<InterviewExperience[]> {
...
async function insertInterviewExperience(
...
async function clearMockData(): Promise<{
  deletedSalaryReports: number;
  deletedInterviewExperiences: number;
  deletedHelpfulVotes: number;
}> {
  ...
  for (const exp of interviewExperiences) {
    await db
      .delete(schema.interviewExperience)
  ...
  for (const vote of helpfulVotes) {
    await db
      .delete(schema.helpfulVote)
      .where(eq(schema.helpfulVote.id, vote.id as string));
  }
...
function parseArgs(): {
  ...
  return {
    ...
    salaryCount:
      parseInt(
        args.find((a) => a.startsWith("--salaries="))?.split("=")[1] || "",
      ) || DEFAULT_SALARY_COUNT,
    interviewCount:
      parseInt(
        args.find((a) => a.startsWith("--interviews="))?.split("=")[1] || "",
...
async function main() {
  ...
  if (replace && !dryRun) {
    ...
    console.log(
      `  Deleted ${deleted.deletedInterviewExperiences} interview experiences`,
    );
    console.log();
  }
  ...
  if (!interviewsOnly) {
    ...
    if (dryRun) {
      ...
      for (const report of salaryReports.slice(0, 5)) {
        console.log(
          `  ${report.mosCode} @ ${report.companySlug}, ${report.location}: $${report.baseSalary.toLocaleString()} (${report.clearanceLevel})`,
        );
      }
      ...
      for (const report of salaryReports) {
        try {
          ...
          if (inserted % 10 === 0) {
            console.log(
              `  Inserted ${inserted}/${salaryReports.length} salary reports`,
            );
          }
        } catch (error) {
      ...
      console.log(
        `  Total inserted: ${inserted}/${salaryReports.length} salary reports`,
      );
    }
    console.log();
  }
  ...
  if (!salariesOnly) {
    ...
    const interviewExperiences =
      await generateAllInterviewExperiences(interviewCount);
    console.log();
    ...
    if (dryRun) {
      ...
      for (const exp of interviewExperiences.slice(0, 5)) {
        console.log(
          `  ${exp.mosCode} @ ${exp.companySlug} - ${exp.roleTitle} (${exp.outcome}, ${exp.difficulty})`,
        );
      }
      ...
      for (const exp of interviewExperiences) {
        try {
          ...
          if (inserted % 10 === 0) {
            console.log(
              `  Inserted ${inserted}/${interviewExperiences.length} interview experiences`,
            );
          }
        } catch (error) {
      ...
      console.log(
        `  Total inserted: ${inserted}/${interviewExperiences.length} interview experiences`,
      );
    }
  }
