...
const SPECIALTIES = [
  { slug: 'aerospace-defense', name: 'Aerospace & Defense', description: 'Aircraft, missiles, satellites', icon: 'mdi:airplane' },
  { slug: 'cybersecurity-it', name: 'Cybersecurity & IT', description: 'Cyber, IT services, cloud', icon: 'mdi:shield-lock' },
  { slug: 'intelligence-analytics', name: 'Intelligence & Analytics', description: 'Intel, data analytics, AI/ML', icon: 'mdi:chart-box' },
  { slug: 'land-systems', name: 'Land Systems', description: 'Vehicles, weapons, munitions', icon: 'mdi:tank' },
  { slug: 'naval-maritime', name: 'Naval & Maritime', description: 'Ships, submarines, maritime systems', icon: 'mdi:ship-wheel' },
  { slug: 'space-systems', name: 'Space Systems', description: 'Satellites, launch, space tech', icon: 'mdi:rocket-launch' },
  { slug: 'professional-services', name: 'Professional Services', description: 'Consulting, engineering services', icon: 'mdi:briefcase' },
  { slug: 'logistics-support', name: 'Logistics & Support', description: 'Base ops, supply chain, MRO', icon: 'mdi:truck-delivery' },
  { slug: 'electronics-sensors', name: 'Electronics & Sensors', description: 'Sensors, communications, EW', icon: 'mdi:radar' },
  { slug: 'research-development', name: 'Research & Development', description: 'FFRDCs, labs, R&D', icon: 'mdi:flask' },
]
 ...
 */
function parseHeadquarters(hq: string): { city: string | null; state: string | null } {
...
async function seedSpecialties(dryRun: boolean): Promise<Map<string, string>> {
  ...
  for (const specialty of SPECIALTIES) {
    if (dryRun) {
      console.log(`  [DRY RUN] Would insert: ${specialty.slug} - ${specialty.name}`)
...
async function seedContractors(
  ...
  for (const contractor of contractors) {
    if (dryRun) {
      console.log(`  [DRY RUN] Would insert: ${contractor.name} (${contractor.slug})`)
      continue
    }
    ...
    for (let i = 0; i < contractor.specialties.length; i++) {
      ...
      if (!specialtyId) {
        console.warn(`    ⚠ Warning: Specialty "${specialtySlug}" not found for ${contractor.name}`)
        continue
      }
