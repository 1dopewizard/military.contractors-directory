...
async function main() {
  ...
  console.log()
  console.log(`Total: ${totalRows} rows from ${results.filter(r => r.success).length}/${results.length} tables`)
  ...
  await writeFile(join(outputDir, '_manifest.json'), JSON.stringify(manifest, null, 2))
}
