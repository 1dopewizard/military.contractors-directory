 ...
 * @note Legacy MOS fields (mosCode, mosId) kept as text for data preservation.
 *       companyId now references contractor table from directory schema.
 */
...
export const salaryReport = sqliteTable(
  "salary_report",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    mosCode: text("mosCode").notNull(), // Legacy: kept as text for historical data
    mosId: text("mosId"), // Legacy: kept as text, no longer a foreign key
...
export const interviewExperience = sqliteTable(
  "interview_experience",
  {
    ...
    questionsAsked: text("questionsAsked", { mode: "json" })
      .$type<string[]>()
      .notNull(),
...
export const helpfulVote = sqliteTable(
  "helpful_vote",
  {
    id: text("id")
      .primaryKey()
  ...
  (table) => [
    ...
    index("helpful_vote_unique_idx").on(
      table.userId,
      table.targetType,
      table.targetId,
    ),
