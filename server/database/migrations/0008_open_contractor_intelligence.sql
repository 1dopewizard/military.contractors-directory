CREATE TABLE IF NOT EXISTS `recipientEntity` (
  ...
  FOREIGN KEY (`contractorId`) REFERENCES `contractor`(`id`) ON UPDATE no action ON DELETE set null
);

CREATE INDEX IF NOT EXISTS `recipientEntity_contractor_idx` ON `recipientEntity` (`contractorId`);
CREATE INDEX IF NOT EXISTS `recipientEntity_normalized_name_idx` ON `recipientEntity` (`normalizedName`);
CREATE UNIQUE INDEX IF NOT EXISTS `recipientEntity_uei_idx` ON `recipientEntity` (`uei`);
